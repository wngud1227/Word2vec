...
class SkipGram:
    def __init__(self, window_size, hidden_unit): #window_size: the number of input word, hidden_unit : dimension of vector
        ...
        self.W_embedding_b = np.random.uniform(low=-0.5, high=0.5, size=(self.vocab_size - 1, self.dimension))
    ...
    def forward(self, contexts, center, id_to_code, id_to_way):
        ...
        for context in contexts:
            code = np.array(id_to_code[context]).reshape(1, -1)   #(1, V-1)
            way = list(id_to_way[context])  #(V-1,)
            score = sigmoid(np.dot(code, np.dot(x, self.W_embedding_b[way].T).T)) #(1, 1)
            loss += -np.log(score.squeeze(0) + 1e-07)
    ...
    def backward(self, lr):
        for (x, context, code, way, score) in self.cache:
            dout = ((score - 1) * code) #(1, V-1)
            dW_out = np.dot(dout.T, x) #(V-1, D)
    ...
    def train(self, epoch):
        ...
        with open('data/dataset/embedding_sg_hf.pkl', 'wb') as f:
...
# with open('./data/dataset/embedding_sg_neg15_2.pkl', 'rb') as f:
#     W_embedding = pickle.load(f)
# accuracy = accuracy(file='./data/dataset/news/test_labeled.pkl', W=W_embedding)
